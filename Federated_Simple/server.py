...
NN_model = bp.NeuralNetwork(description,num_inputs,"cross_entropy_sigmoid", data_inputs, data_outputs, learning_rate=0.001)
            ...
            if (("data" in received_data.keys()) and ("subject" in received_data.keys())):
                ...
                if subject == "echo":
                    ...
                    try:
                        ...
                        counter+=1
                        error = model.calc_accuracy(data_inputs, data_outputs, "classification")
                        print("Error from {info} = {error}".format(error=error, info=self.client_info))
