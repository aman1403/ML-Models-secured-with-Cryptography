...
if data_len == 223:
    data = df[int(len(df)/6):int(2*len(df)/6)]
else:
    data = df[int(4*len(df)/6):int(5*len(df)/6)]
    ...
    for i in range(layers):
        ...
        for a in range(len(bias_list[i])):
            for b in range(len(bias_list[i][a])):
                ...
                encrypted_bias2 = shamir_share(int(encrypted_bias))
                for z in range(15):
                    encrypted_bias_list.append(encrypted_bias2[z])
                n = n + 1
...
def send_data():
    while True:
        ...
        # print("0-15th elelment:",encrypted_bias_list[0:15])
...
def recv_data():
    global data, ready_to_send , weight_data, weight_dat, bias_dat, bias_data, encrypted_weight_list, encrypted_bias_list
    sema.acquire()
    ...
    if node_address not in ready_to_send:
        ...
        if len(weight_data) == 0:
            ...
            # print("recv3",response_data["encrypted_bias_shares"][0:15])
...
def recv_data_server():
    global data, ready_to_send, s_model, encrypted_weight_list, encrypted_bias_list, thr, weight_list, bias_list, count, model
