...
if data_len == 223:
    data = df[int(2*len(df)/6):int(3*len(df)/6)]
else:
    data = df[int(5*len(df)/6):]
...
def example_serialize(layers, weight_list1, bias_list1):
    encrypted_weight_list = []
    encrypted_bias_list = []
    m = 0
    n = 0
    ...
    for i in range(layers):
        for j in range(len(weight_list1[i])):
            for k in range(len(weight_list1[i][j])):
                encrypted_weight = weight_list1[i][j][k]
        ...
        for a in range(len(bias_list1[i])):
            for b in range(len(bias_list1[i][a])):
                encrypted_bias = bias_list1[i][a][b]
...
def send_data():
    start = time.time()
    while True:
        global data, CONNECTED_NODE_ADDRESS,encrypted_weight_list,encrypted_bias_list
         # sending
        weight_list = []
        bias_list = []
        for i in range(len(model.layers)):
        ...
        for peer in CONNECTED_NODE_ADDRESS:
            if int(peer[-1]) == int(myaddress[-1]) - 2:
                l = 0
            else:
                l = 5
            v = l
        ...
        print('Distributed shares to clients')
        time.sleep(20)
    return 'Sent', 200
#-----------------------------------------------------------------------------------------------------------------------------------------------------
...
def recv_data():
    global data, ready_to_send , weight_data, weight_dat, bias_dat, bias_data, encrypted_weight_list, encrypted_bias_list,weight_list,bias_list
    sema.acquire()
    layers = len(model.layers)
    for i in range(len(model.layers)):
        weight_list.append(model.layers[i].W)
    ...
    # print("0-15th element",encrypted_bias_list[0:15])
    ...
    if node_address not in ready_to_send:
        ...
        if len(weight_data) == 0:
            ...
            # print("recv1",response_data["encrypted_bias_shares"][0:15])
        else:
            ...
            # print("recv2",response_data["encrypted_bias_shares"][0:15])
...
def recv_data_server():
    global data, ready_to_send, s_model, encrypted_weight_list, thr, encrypted_bias_list, weight_list, bias_list, count, model
